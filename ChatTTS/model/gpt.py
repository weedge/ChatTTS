...
class GPT_warpper(nn.Module):
        ...
        **kwargs,
        ...
        self.head_text = weight_norm(
            nn.Linear(
                ...
                bias=False),
            name='weight')
        self.head_code = nn.ModuleList([weight_norm(nn.Linear(
            self.model_dim, num_audio_tokens, bias=False), name='weight') for i in range(self.num_vq)])
    ...
    def get_emb(self, input_ids, text_mask, **kwargs):
        ...
        emb = torch.zeros((input_ids.shape[:-1]) + (emb_text.shape[-1],),
    ...
    def generate(
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(max_new_token)):
                ...
                if i == 0:
                    ...
                    if infer_text:
                        ...
                        code_emb = [self.emb_code[i](model_input['input_ids'][:, :, i])
                ...
                with P.cached():
                    if infer_text:
                        ...
                        logits = torch.stack([self.head_code[i](hidden_states)
                                             for i in range(self.num_vq)], 3)
                ...
                if not infer_text:
                    ...
                    inputs_ids = torch.cat(
                        [inputs_ids, idx_next.unsqueeze(-1).expand(-1, -1, self.num_vq)], 1)
            ...
            inputs_ids = [inputs_ids[idx, start_idx: start_idx + i]
